...
interface IKEYMintGateLegacyV3 {
    ...
    function publicMintedTotal() external view returns (uint256);
}

/// @title KEY Mint Gate V3
/// @notice Replacement mint gate for the 10-mints-per-identity KEYSPACE rule.
/// @dev Keeps the EIP-712 domain compatible with earlier gates so backend typed-data signing stays unchanged.
contract KEYMintGateV3 {
    ...
    IKEYTokenV3 public immutable token;
    IKEYTreasuryVaultV3 public immutable treasuryVault;
    address public immutable legacyMintGate;
    address[] private additionalLegacyMintGates;
    ...
    constructor(
        ...
        token = IKEYTokenV3(token_);
        treasuryVault = IKEYTreasuryVaultV3(treasuryVault_);
        legacyMintGate = legacyMintGate_;
        for (uint256 i = 0; i < additionalLegacyMintGates_.length; i += 1) {
            require(additionalLegacyMintGates_[i] != address(0), "bad legacy gate");
            additionalLegacyMintGates.push(additionalLegacyMintGates_[i]);
        }
        owner = msg.sender;
    ...
    function legacyMintGateCount() public view returns (uint256) {
        return 1 + additionalLegacyMintGates.length;
    }

    function legacyMintGateAt(uint256 index) public view returns (address) {
        if (index == 0) return legacyMintGate;
        return additionalLegacyMintGates[index - 1];
    }

    function legacyWalletMints(address minter) public view returns (uint256) {
        uint256 rawSum = 0;
        uint256 totalAwareMax = 0;
        for (uint256 i = 0; i < legacyMintGateCount(); i += 1) {
            IKEYMintGateLegacyV3 gate = IKEYMintGateLegacyV3(legacyMintGateAt(i));
            try gate.walletMintsTotal(minter) returns (uint256 total) {
                if (total > totalAwareMax) totalAwareMax = total;
            } catch {
                rawSum += gate.walletMints(minter);
            }
        }
        return totalAwareMax > rawSum ? totalAwareMax : rawSum;
    }

    function walletMintsTotal(address minter) public view returns (uint256) {
        return legacyWalletMints(minter) + walletMints[minter];
    }

    function walletRemainingMints(address minter) public view returns (uint256) {
        uint256 minted = walletMintsTotal(minter);
    ...
    function publicMintedTotal() public view returns (uint256) {
        ...
        for (uint256 i = 0; i < legacyMintGateCount(); i += 1) {
            IKEYMintGateLegacyV3 gate = IKEYMintGateLegacyV3(legacyMintGateAt(i));
            try gate.publicMintedTotal() returns (uint256 total) {
                if (total > totalAwareMax) totalAwareMax = total;
            } catch {
                rawSum += gate.publicMinted();
            }
        }
        uint256 legacyMinted = totalAwareMax > rawSum ? totalAwareMax : rawSum;
        return legacyMinted + publicMinted;
    }

    function legacyUsedProofId(bytes32 id) public view returns (bool) {
        for (uint256 i = 0; i < legacyMintGateCount(); i += 1) {
            if (IKEYMintGateLegacyV3(legacyMintGateAt(i)).usedProofId(id)) return true;
        }
        return false;
    }

    function legacyUsedPublicKeyHash(bytes32 publicKeyHash) public view returns (bool) {
        for (uint256 i = 0; i < legacyMintGateCount(); i += 1) {
            if (IKEYMintGateLegacyV3(legacyMintGateAt(i)).usedPublicKeyHash(publicKeyHash)) return true;
        }
        return false;
    }
    ...
    function mintWithAttestation(MintAttestation calldata a, bytes calldata signature) external payable nonReentrant {
        ...
        require(!legacyUsedProofId(id) && !usedProofId[id], "proof used");
        require(!legacyUsedPublicKeyHash(a.publicKeyHash) && !usedPublicKeyHash[a.publicKeyHash], "public key used");
        require(walletMintsTotal(a.recipient) < WALLET_CAP, "wallet cap reached");
