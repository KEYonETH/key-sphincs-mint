...
interface IKEYTreasuryVault {
    function depositMintFee(address minter) external payable;
}

/// @title KEY Mint Gate
/// @notice Receives ETH mint fee and mints KEY only with a valid EIP-712 backend attestation.
/// @dev The contract recomputes reward tier from rewardHash so the backend cannot approve arbitrary rewards.
contract KEYMintGate {
    ...
    IKEYToken public immutable token;
    IKEYTreasuryVault public immutable treasuryVault;
    ...
    constructor(address token_, address treasuryVault_, address attestationSigner_) {
        ...
        token = IKEYToken(token_);
        treasuryVault = IKEYTreasuryVault(treasuryVault_);
        owner = msg.sender;
