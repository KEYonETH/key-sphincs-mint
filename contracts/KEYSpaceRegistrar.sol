...
/// @title KEYSPACE Registrar
/// @notice Origin claim contract for KEY minters. It verifies mint attestations on-chain and locks KEY as KeyBond.
contract KEYSpaceRegistrar is Ownable {
    ...
    IKEYBondToken public immutable keyToken;
    IKEYIdentityMint public immutable identity;
    ...
    constructor(
        ...
        keyToken = IKEYBondToken(keyToken_);
        identity = IKEYIdentityMint(identity_);
    ...
    function claimOrigin(
        ...
        if (!allowedMintGate[mintGate]) revert InvalidMintGate();
        if (attestation.recipient != msg.sender) revert InvalidAttestation();
        if (claimedByWallet[msg.sender]) revert ClaimAlreadyUsed();
        if (!_isValidName(name)) revert InvalidName();

        bytes32 proof = proofId(attestation);
        if (claimedProofId[proof]) revert ProofAlreadyClaimed();

        IKEYMintGateOrigin gate = IKEYMintGateOrigin(mintGate);
        if (gate.walletMints(msg.sender) == 0) revert InvalidAttestation();
        if (!gate.usedProofId(proof)) revert InvalidAttestation();
        if (attestation.rewardHash != recomputeRewardHash(attestation)) revert InvalidAttestation();
        if (attestation.rewardAmount != rewardForHash(attestation.rewardHash)) revert InvalidAttestation();
        if (_recover(_typedDataHash(attestation, mintGate), signature) != gate.attestationSigner()) revert InvalidAttestation();

        uint8 rank = rankForReward(attestation.rewardAmount);
        if (bytes(name).length < minNameLengthForRank(rank)) revert NameTooShort();

        claimedByWallet[msg.sender] = true;
        claimedProofId[proof] = true;

        if (!keyToken.transferFrom(msg.sender, address(this), attestation.rewardAmount)) revert TransferFailed();

        tokenId = identity.mintIdentity(msg.sender, name, rank, attestation.rewardAmount, msg.sender, proof);
        keyBondByTokenId[tokenId] = attestation.rewardAmount;

        emit IdentityClaimed(msg.sender, tokenId, name, rank, attestation.rewardAmount);
    }
